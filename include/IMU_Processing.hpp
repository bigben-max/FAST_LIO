...
class ImuProcess {
 ...
 private:
  ...
  std::deque<ImuMeasurement> v_imu_;
...
void ImuProcess::UndistortPcl(
  ...
  const double &imu_beg_time = v_imu.front().time_second;
  const double &imu_end_time = v_imu.back().time_second;
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    if (tail.time_second < last_lidar_end_time_) continue;
    ...
    // fout_imu << setw(10) << head.time_second - first_lidar_time <<
    // " " << angvel_avr.transpose() << " " << acc_avr.transpose() << endl;

    acc_avr = acc_avr * G_m_s2 / mean_acc.norm();  // - state_inout.ba;

    if (head.time_second < last_lidar_end_time_) {
      dt = tail.time_second - last_lidar_end_time_;
      // dt = tail.time_second - pcl_beg_time;
    } else {
      dt = tail.time_second - head.time_second;
    }
    ...
    double &&offs_t = tail.time_second - pcl_beg_time;
